    ...
    func requestTransaction(_ transaction: CXTransaction, action: String = "") {
        self.controller.request(transaction) { error in
            guard error == nil else {
                print("Error Requesting Transaction \(transaction) \(action): \(String(describing: error))")
                return
            }
...
extension CXCallManager: CXProviderDelegate {
    ...
    func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        ConnectionManager.connect { [weak call] error in
    ...
    func showCallController(with call: DirectCall) {
        DispatchQueue.main.async { [weak self] in
            ...
            if let topViewController = UIViewController.topViewController {
                topViewController.present(callingViewController, animated: true, completion: nil)
            }
        }
    }
}
